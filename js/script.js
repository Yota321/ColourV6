...
function renderPalettes(customPalettes) {
  // Filter palettes based on current filter, search, and view
  let filteredPalettes = customPalettes || colorPalettes;
  
  if (currentFilter) {
    if (currentFilter.startsWith('color:')) {
      // Special case for color filtering
      const hexCode = currentFilter.split(':')[1];
      // No need to filter if we already have custom palettes
      if (!customPalettes) {
        filteredPalettes = findPalettesWithColor(hexCode);
      }
    } else {
...
function renderColors() {
  ...
  filteredColors.forEach(color => {
    ...
    // Add click event to show palettes with this color
    colorCard.addEventListener('click', () => {
      // Find palettes containing this color
      const palettesWithColor = findPalettesWithColor(color.hex);
      
      if (palettesWithColor.length > 0) {
        // Switch to palettes view
        switchView('palettes');
        
        // Filter palettes to show only those containing this color
        currentFilter = `color:${color.hex}`;
        
        // Update UI to show filtered palettes
        renderPalettes(palettesWithColor);
        
        // Show notification
        showNotification(`Showing palettes containing ${color.hex}`);
      } else {
        // If no palettes found, just show the color detail
...
// Find palettes containing a specific color
function findPalettesWithColor(hexCode) {
  return colorPalettes.filter(palette => 
    palette.colors.some(color => 
      color.hex.toUpperCase() === hexCode.toUpperCase()
    )
  );
}
